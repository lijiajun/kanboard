...
token=`mysql -uroot -Dkanboard -e"select token from projects where id = $1;" | sed -n '2p'`
if [ -z "$token" ]; then
    echo "项目编号$1对应的项目不存在或未公开，请输入其他项目编号。"
    exit 1
fi

project_name=`mysql -uroot -Dkanboard -e"select name from projects where id = $1;" | sed -n '2p'`
